...
# ========== FIX:  Two simple variables to remember the color ==========
saved_colour = None      # This will store the current result color
has_result = False       # This tells us if we have a result showing
# =====================================================================
...
def on_reset(event):
    ...
    global has_result, saved_colour
...
def on_calculate(event):
    ...
    global saved_colour, has_result
    ...
    # ========== FIX: Save the color so we can use it later ==========
    saved_colour = colour
    has_result = True
    # ================================================================
...
# ========== FIX: New function to keep colors when window resizes ==========
def on_resize(event):
    """This runs when window size changes.  It keeps the colors correct."""
    event.Skip()  # Let normal resize happen first

    if has_result and saved_colour is not None:
        bmi_label.SetForegroundColour(saved_colour)
        category_title_label.SetForegroundColour(saved_colour)
        category_value_label.SetForegroundColour(saved_colour)
        tip_label.SetForegroundColour(saved_colour)
        panel.Refresh()
# ==========================================================================
...
frame = wx.Frame(None, title="BMI Calculator (Enhanced)", size=(520, 480))
...
main_sizer.Add(history_info_label, 0, wx. ALIGN_CENTER | wx.TOP | wx.BOTTOM, 6)
...
# ========== FIX: Listen for window resize ==========
frame.Bind(wx.EVT_SIZE, on_resize)
# ===================================================

# ========== FIX: Make layout correct before showing ==========
panel.Layout()
# =============================================================
